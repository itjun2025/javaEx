...
/**
 * UnCheckedException
 * 	- RuntimeException  을 상속하고 있는 예외들 ==> 실행예외
 ...
 *  - ArrayIndexOutOfBoundsExceptiony : 배열의 접근에 잘봇된 인덱스 값을 이용한 경우
 ...
 *  - ClassCastExceptions : 잘못된 Cast연산자 사용시   
 * @author user
 *
 */
...
//		 d.method2();
	...
	public void method3() {
		...
		// 배열을 생성하는데 배열의 사이즈를 음수로 설정할경우
		// NegativeArraySizeException 이 발생할 소지가 있어서
		// 입력된 값이 양수인지 체크 합니다.
		if (size<0) {
		...
		// 0 ~ n-1
		// ArrayIndexOutOfBoundsExceptiony 
		
		try {
			// 방번호는 0부터 시작하므로
			// 배열의 생성길이 size보다 1 작은수 까지 참조가 가능하다
			// 유효하지않은 방번호에 접근하려고 할경우
			// ArrayIndexOutOfBoundsExceptiony 오류가 발생
			int num = arr[size];
		} catch (ArrayIndexOutOfBoundsException e) {
			
			System.out.println("잘못된배열의 길이가 입력되었습니다");
		}
